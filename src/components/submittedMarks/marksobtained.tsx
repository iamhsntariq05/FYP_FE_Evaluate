...
export default function Marksobtained() {
  ...
  const columns: GridColDef[] = [
    ...
    { field: "deadline", headerName: "Date", width: 220 },
    ...
    //           return (
    //             <li key={i + rubric.obtained_score} style={{ lineHeight: "20px" }}>
    //               ({rubric.score})
    //             </li>
    //           );
    //         })}
  ...
  const fetchDeliverables = async () => {
    try {
      ...
      const res = response.data.filter((dt: any) => {
        if (dt.student_ID === localStorage.getItem("id") && dt.markedObtain === true) {
          return dt;
        }
      });
      setRows(res);
